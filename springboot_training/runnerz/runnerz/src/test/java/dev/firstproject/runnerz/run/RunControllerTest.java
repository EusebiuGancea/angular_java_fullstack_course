...
import dev.firstproject.runnerz.run.RunController;
...
class RunControllerTest {
    ...
    void shouldCreateNewRun() throws Exception {
        var run = new Run(
                ...
                "test",
                LocalDateTime.now(),
                LocalDateTime.now().plusMinutes(30), // completedOn is now after startedOn
                1,
                Location.INDOOR
        );
    ...
    public void shouldDeleteRun() throws Exception {
        Run run = new Run(1, "Monday Morning Run", LocalDateTime.now(), LocalDateTime.now().plus(30, ChronoUnit.MINUTES), 3, Location.INDOOR);
        when(repository.findById(1)).thenReturn(Optional.of(run)); // Mocking the findById to return the run
